    ...
    d(new FancyLineEditPrivate(this))
{
    ...
    connect(d->m_iconbutton[Left], SIGNAL(clicked(bool)), this, SLOT(iconClicked()));
    connect(d->m_iconbutton[Right], SIGNAL(clicked(bool)), this, SLOT(iconClicked()));
...
void FancyLineEdit::setHistoryCompleter(const QString &historyKey, bool restoreLastItemFromHistory)
{
    ...
    // "a bit".
    connect(this, SIGNAL(QLineEdit::editingFinished),
            this, SLOT(FancyLineEdit::onEditingFinished), Qt::QueuedConnection);
}
...
void FancyLineEdit::setFiltering(bool on)
{
    ...
    if (on) {
        ...
        connect(this, SIGNAL(FancyLineEdit::rightButtonClicked), this, SLOT(QLineEdit::clear));
    } else {
        disconnect(this, SIGNAL(FancyLineEdit::rightButtonClicked), this, SLOT(QLineEdit::clear));
    }
}
...
{
    const qreal pixmapRatio = 1.0; //m_pixmap.devicePixelRatio();

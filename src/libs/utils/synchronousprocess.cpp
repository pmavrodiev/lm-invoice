    ...
    d(new SynchronousProcessPrivate)
{
    ...
    connect(&d->m_timer, SIGNAL(QTimer::timeout), this, SLOT(SynchronousProcess::slotTimeout));
    //connect(&d->m_process,
    //        static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(SIGNAL(QProcess::finished)),
    //        this, SLOT(SynchronousProcess::finished));
    //connect(&d->m_process, static_cast<void (QProcess::*)(QProcess::ProcessError)>(SIGNAL(QProcess::error)),
    //        this, SLOT(SynchronousProcess::error));
    connect(&d->m_process,SIGNAL(QProcess::finished),this, SLOT(SynchronousProcess::finished));
    connect(&d->m_process, SIGNAL(QProcess::error), this, SLOT(SynchronousProcess::error));
    
    connect(&d->m_process, SIGNAL(QProcess::readyReadStandardOutput),
            this, SLOT(SynchronousProcess::stdOutReady));
    connect(&d->m_process, SIGNAL(QProcess::readyReadStandardError),
            this, SLOT(SynchronousProcess::stdErrReady));
}
                                             ...
                                             bool showTimeOutMessageBox)
{
    ...
    do {
        ...
        hasData = hang && showTimeOutMessageBox && !askToKill(p.workingDirectory()/*p.program()*/);

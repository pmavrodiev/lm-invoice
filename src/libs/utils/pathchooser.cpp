...
PathChooserPrivate::PathChooserPrivate(PathChooser *chooser) :
    ...
    m_acceptingKind(PathChooser::File),
    ...
    d(new PathChooserPrivate(this))
{
    ...
    connect(d->m_lineEdit, SIGNAL(validReturnPressed()), this, SLOT(slot_returnPressed()));
    connect(d->m_lineEdit, SIGNAL(textChanged(const QString &)), this, SLOT(slot_changed(const QString &)));
    connect(d->m_lineEdit, SIGNAL(validChanged(bool)), this, SLOT(slot_validChanged(bool)));
    connect(d->m_lineEdit, SIGNAL(editingFinished()), this, SLOT(slot_editingFinished()));
    connect(d->m_lineEdit, SIGNAL(textChanged(const QString &)), this, SLOT(slotTextChanged()));

    d->m_lineEdit->setMinimumWidth(120);
    d->m_hLayout->addWidget(d->m_lineEdit);
    d->m_hLayout->setSizeConstraint(QLayout::SetMinimumSize);

    addButton(browseButtonLabel(),/*receiver=*/this,/*slot function when clicked=*/ SLOT(slotBrowse()));
...
void PathChooser::slot_changed(const QString& text) {
  qDebug() << "Emitted s_changed";
}

void PathChooser::slot_editingFinished() {
  qDebug() << "Emitted s_editingFinished";
}

void PathChooser::slot_returnPressed() {
  qDebug() << "Emitted s_returnPressed";
}

void PathChooser::slot_validChanged(bool validState) {
  qDebug() << "Emitted s_validChanged";
}
